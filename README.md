...
v0.10 解决了“所有视图虽然共享状态，却不能在拖动时同时看见”的逻辑缺陷。单层薄膜旗舰现在采用：

```text
分析模块选择坞 | 主实验台 | 同步分析区 | 参数栏
```

- 主光路、拖动把手、必要实时量和参数控制始终保留；
- 实验装置、界面相位、多束相量、光谱、相图、盲样反演、公式映射和验证默认隐藏；
- 用户通过语义化复选框按需挂载分析模块；
- 已选模块与主光路在同一状态、同一时刻更新；
- 推荐同时开启 1–2 个，最多开启 4 个，防止重新退化成不可读的小图仪表盘；
- 提供“相位机制、光谱分析、实验反演、四窗全景”预设；
- 任一窗口可放大或直接关闭；
- 挂载后自动重建高清 Canvas backing store；
- 超宽屏保持主实验台和同步分析区同时可见，中小屏逐级转为响应式布局。

详细设计依据、否决方案、同步协议和全项目迁移规则见 [`docs/SYNCHRONIZED_WORKBENCH_V010.md`](docs/SYNCHRONIZED_WORKBENCH_V010.md)。

### v0.9：页面结构与学院士式审查

v0.9 建立了模型页语义结构：板块目的、作用、建议操作、真实标题层级和模型边界。该规范仍然保留，但薄膜旗舰已从线性项目栏进一步升级为 v0.10 的同步工作台。
...
- 近场/远场、标量/矢量、近轴/大角度等近似必须明确标注。
- 旗舰模型修改必须通过物理不变量、页面结构、高清绘图和工作台合同测试。
