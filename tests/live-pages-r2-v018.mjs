...
const requiredResources=['assets/css/r2-legibility-v016.css','assets/css/r2-wide-sync-formula-v018.css','assets/js/r2-legibility-v016.js','assets/js/r2-canvas-text-audit-v016.js'];
...
async function focusWorkspace(){
  await page.evaluate(()=>{
    const workspace=document.querySelector('.rfw-workspace');const topbar=document.querySelector('.topbar');
    if(!workspace)return;const y=workspace.getBoundingClientRect().top+scrollY-(topbar?.offsetHeight||0)-6;
    scrollTo({top:Math.max(0,y),behavior:'instant'});
  });
  await page.waitForTimeout(700);
}
async function selectProofPreset(){
...
async function dragLogical(locator,start,end){
  const box=await locator.boundingBox();if(!box)throw new Error('public main canvas has no box');
...
try{
  ...
  for(let attempt=1;attempt<=30;attempt++){
    result.attempts=attempt;const url=`${baseUrl}?r2v018=${Date.now()}-${attempt}`;
    const response=await page.goto(url,{waitUntil:'networkidle',timeout:60000});result.httpStatus=response?.status()||null;
    const html=await page.content();result.resources=Object.fromEntries(requiredResources.map(resource=>[resource,html.includes(resource)]));
  ...
  await page.waitForTimeout(1800);await focusWorkspace();
  ...
  result.layout=await page.evaluate(()=>{
    ...
    return{primary:rect('.rfw-primary-column'),analysis:rect('.rfw-analysis-column'),mechanism:rect('[data-module-id="mechanism"]'),observable:rect('[data-module-id="observable"]'),columns:getComputedStyle(workspace).gridTemplateColumns,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth};
  });
  const {primary,analysis}=result.layout;
  if(!primary||!analysis)result.assertions.push('public primary or analysis missing');
  else{
    if(analysis.x<primary.right+12)result.assertions.push(`public R2 still vertical: ${primary.right}/${analysis.x}`);
  ...
  for(const key of ['mechanism','observable'])if(!result.layout[key]||result.layout[key].width<590)result.assertions.push(`public ${key} too narrow ${result.layout[key]?.width||0}px`);
  if(result.layout.overflow>2)result.assertions.push(`public horizontal overflow ${result.layout.overflow}px`);
  await page.screenshot({path:`${outDir}/r2-live-1790x896-wide-compare.png`,fullPage:false});
  ...
  await dragLogical(main,{x:330,y:395},{x:280,y:350});await page.waitForTimeout(700);
  ...
  await selectProofPreset();
  const formulaModule=page.locator('[data-module-id="derivation"]');
  await formulaModule.evaluate(node=>node.scrollIntoView({block:'center',inline:'nearest',behavior:'instant'}));await page.waitForTimeout(700);
  result.formula=await formulaModule.evaluate(module=>{
    const mr=module.getBoundingClientRect();
    const formulas=[...module.querySelectorAll('.rfw-equation mjx-container')].filter(node=>{const r=node.getBoundingClientRect();return r.width>0&&r.height>0}).map(node=>{
      const r=node.getBoundingClientRect();const er=node.closest('.rfw-equation')?.getBoundingClientRect()||mr;
      return{text:(node.textContent||'').trim(),fontSize:parseFloat(getComputedStyle(node).fontSize),x:r.x,y:r.y,width:r.width,height:r.height,leftSafety:r.left-er.left,rightSafety:er.right-r.right,topSafety:r.top-er.top,bottomSafety:er.bottom-r.bottom};
    });
    const steps=[...module.querySelectorAll('.rfw-derivation-list article')].map(node=>node.getBoundingClientRect().height);
    return{module:{x:mr.x,y:mr.y,width:mr.width,height:mr.height},formulas,steps};
  });
  ...
  for(const formula of result.formula.formulas||[]){
    ...
    if(formula.leftSafety<-1||formula.rightSafety<-1||formula.topSafety<-1||formula.bottomSafety<-1)result.assertions.push(`public formula clipped: ${formula.text}`);
  }
  for(const height of result.formula.steps||[])if(height<62)result.assertions.push(`public formula row too short ${height}px`);
...
}finally{await fs.writeFile(`${outDir}/metrics.json`,JSON.stringify(result,null,2));await browser.close();}
console.log(JSON.stringify(result,null,2));if(result.errors.length||result.assertions.length)process.exit(1);
